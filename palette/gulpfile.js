...
gulp.task('sprite', () => gulp.src('./build/**/*.svg')
  .pipe(svgmin({
    plugins: [{
      removeViewBox: false,
    }],
...
gulp.task('copy', gulp.series('scripts', 'style', () => gulp.src([
  'src/**/*.{html,ico}',
...
gulp.task('tinypng', gulp.series(async () => gulp.src('build/img/**/*.{jpg,png,jpeg,gif}')
  .pipe(tinypng({
...
gulp.task('imagemin', gulp.series(() => gulp.src('build/img/**/*.{jpg,png,jpeg,gif}')
  .pipe(imagemin([
    imagemin.optipng({
      optimizationLevel: 3,
    }),
